...
final class FocusController: NSObject {
    ...
    init(videoDeviceInput: AVCaptureDeviceInput, videoPreviewView: GLKView, videoPreviewFrame: CGRect?, sessionQueue: dispatch_queue_t) {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(FocusController.subjectAreaDidChange(_:)), name: AVCaptureDeviceSubjectAreaDidChangeNotification, object: videoDeviceInput.device)

        tapGestureRecognizer.addTarget(self, action: #selector(FocusController.tapped(_:)))
